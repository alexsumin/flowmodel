    ...
    public void create() {
        try {
            ...
            paragraphConfig2.setText(
                    "Входные данные.\r" +
                            "Тип материала : Полипропилен.\r" +
                            "1. Геометрические параметры канала.\r" +
                            "\t1.1 Ширина, м. W = " + values.get(0) + "\r" +
                            "\t1.2 Длина, м. L =" + values.get(1) + "\r" +
                            "\t1.3 Глубина, м. H = " + values.get(2) + "\r" +
                            "2. Параметры свойств материала." + "\r" +
                            "\t2.1 Плотность, кг/м^3. ρ = " + values.get(3) + "\r" +
                            "\t2.2 Удельная теплоёмкость, Дж/(кг*°C). c =" + values.get(4) + "\r" +
                            "\t2.3 Температура плавления, °C. T0 =" + values.get(5) + "\r" +
                            "3. Режимные параметры." + "\r" +
                            "\t3.1 Скорость движения крышки, м/с. Vu = " + values.get(6) + "\r" +
                            "\t3.2 Температура крышки, °C. Tu =" + values.get(7) + "\r" +
                            "4. Эмпирические коэффициенты математической модели." + "\r" +
                            "\t4.1 Коэффициент консистенции материала, Па*с^n. µ0 = " + values.get(8) + "\r" +
                            "\t4.2 Температурный коэффициент вязкости, 1/°C. b = " + values.get(9) + "\r" +
                            "\t4.3 Температура приведения, °C. Tr = " + values.get(10) + "\r" +
                            "\t4.4 Индекс течения материала, -. n = " + values.get(11) + "\r" +
                            "\t4.5 Коэффициент теплоотдачи от крышки канала к материалу, Вт/(м^2*°C). αu = " + values.get(12) + "\r" +
                            "Температура продукта = " + values.get(13) + "\r" +
                            "Вязкость продукта =" + values.get(14) + "\r" +
                            "Производительность канала" + values.get(15) + "\r");
